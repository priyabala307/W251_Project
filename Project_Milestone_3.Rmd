...
-   Vaccination dataset 
    ...
    -   Recode values to be consistent with morbidity datasets so they can be joined - DONE

    -   Select vaccination time frame that fits best with morbidity data and subset appropriately - DONE

    -   Select demographic strata(s) of interest - DONE
...
Now that we have created an aggregated ca_vax dataset that is only for
Alameda County in the April 1st 2023 quarter with age categories as the chosen
demographic characteristic, we can combine the California vaccine dataset with the simulated dataset.
