...
View(subset_data_age_17)

ca_vax_county_age_no17 <- ca_vax_county %>%
  filter(age_cat != "0-17")

View(ca_vax_county_age_no17)

ca_vax_county_age_no17 <- ca_vax_county_age_no17[,-c(2,3)]
```

```{r}
# Assuming your dataset is named ca_vax_county, you can use the following code to aggregate the data into a single row.
...
View(ca_vax_county_final)

# The observations in the 
```

Now that we have created an aggregated ca_vax dataset that is only for the Alameda county in the April 1st 2023 quarter with age categories as the chosen sociodemographic characteristic, we can combine the California vaccine dataset with the simulated dataset.

```{r}
# Combining the two final datasets to create one final dataset. 

# The ca_vax_county_final dataset has its county_name variables as Alameda rather than Alameda County, thus we are adding additional cleaning to make the merge of the combined datasets easier. 
...
merged_final_alameda <- merged_final_alameda[,c(2,1,3:20)]
print(merged_final_alameda)
